...
# Unicode font (Traditional Chinese) for PDFs
FONT_URL  = "https://github.com/googlefonts/noto-cjk/raw/main/Sans/TTF/TW/NotoSansTC-Regular.ttf"
FONT_DIR  = "./fonts"
...
# ─────────────────────────────────────────────────────────────
# Font helpers for Chinese PDFs
def ensure_font(path: str = FONT_PATH, url: str = FONT_URL):
    """Download NotoSansTC-Regular.ttf if not present."""
    ...
    if not os.path.exists(path):
        try:
            ...
            st.warning(f"Couldn't fetch NotoSansTC font: {e}")
...
# Ensure Unicode font is available for any PDF output
ensure_font()
...
if "chat" not in st.session_state:
    st.session_state.chat = [
        {"role": "system", "content":
         ...
         "- Preferred dataframe for comparisons is `long_df` (vertical stack of all selected tables with a `_table` column).\n"
         "- You may also use per-table variables, but ONLY those listed in `table_vars` (keys only). Do NOT invent names.\n"
         "- When plotting multiple tables, prefer a common x-axis passed as `x_axis` if available.\n"
         "- Prefer matplotlib for charts. Avoid seaborn/plotly/fpdf unless explicitly requested.\n"
         "- When generating PDFs with Chinese text, always create the PDF via make_unicode_pdf().\n"
         "- Output Python code in triple backticks ```python ...``` and, when possible, create `fig` for the plot."}
    ]
...
user_q = st.text_area("Your question / request:", placeholder="e.g., Plot total by Month for each table using long_df.")
run_btn = st.button("Send")

def build_user_payload(question: str, locals_env: dict, per_table_rows=50) -> str:
    # Compact sample of data + allowed variable names and columns
    samples = []
...
{df.head(per_table_rows).to_csv(index=False)}""")
    ...
    return (
        f"{question}\n\n"
        f"Allowed per-table variables (use keys exactly, do NOT invent new names): {allowed}\n"
...
if run_btn and user_q.strip():
    user_payload = build_user_payload(user_q, safe_locals, per_table_rows=50)
    st.session_state.chat.append({"role": "user", "content": user_payload})
