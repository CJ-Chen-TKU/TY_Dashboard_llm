...
# Local font (Traditional Chinese) for PDFs and charts
FONT_DIR  = "./fonts"
FONT_PATH = os.path.join(FONT_DIR, "NotoSansTC-Regular.otf")  # change to .ttf if that’s your file

def ensure_font(path: str = FONT_PATH):
    """Validate local font exists and is non-empty."""
    if not os.path.isfile(path):
        st.error(f"Chinese font not found at: {os.path.abspath(path)}")
        st.stop()
    size = os.path.getsize(path)
    if size < 10_000:  # valid OTF/TTF usually > 100 KB
        st.error(f"Chinese font seems corrupted or too small ({size} bytes)")
        st.stop()
...
# ===== Ensure font ready at app start =====
ensure_font()                          # check file exists & size
FONT_OK = enable_matplotlib_chinese(FONT_PATH)  # enable for charts globally
fp = get_tc_fontprops()                # ready-to-use FontProperties for titles/labels
# st.caption(f"[Font] {os.path.abspath(FONT_PATH)} | enabled={FONT_OK}")  # optional debug
...
Enable 'Allow auto-install missing libs' in the sidebar OR add them to requirements.txt."
    ...
    with contextlib.redirect_stdout(io.StringIO()) as buf:
        try:
            # Ensure matplotlib uses the Chinese font if plotting happens
...
safe_locals = {
    ...
    "get_tc_fontprops": get_tc_fontprops,
    # ▼ add these two:
    "fp": fp,
    "FONT_OK": FONT_OK,
}
...
- ALWAYS use the provided `fp` (already available), or call get_tc_fontprops() if needed.
