...
def ensure_font(path: str = FONT_PATH, url: str = None):
    """Validate local font; do NOT download."""
    if not os.path.exists(path):
        st.error(f"Chinese font not found at: {path}. Please place it there (otf/ttf).")
    else:
        st.info(f"Using Chinese font at: {os.path.abspath(path)}")
...
safe_locals = {
    ...
    "FONT_PATH": FONT_PATH,
    # expose font helpers for GPT code
