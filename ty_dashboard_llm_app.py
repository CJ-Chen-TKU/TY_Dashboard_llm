...
# - Fully dynamic: Workspace → Dataset → Tables
...
# ──────────────────────────────────────────────────────────────────────────────
# Power BI helpers (fully dynamic)
...
@st.cache_data(show_spinner=False, ttl=300)
def list_tables(token: str, workspace_id: str, dataset_id: str) -> list[str]:
...
with st.spinner("Loading tables…"):
    tables = list_tables(token, workspace_id, dataset_id)
