...
# ✅ Only allow these 5 specific tables
allowed_tables = {
    "F-①電子零組件製造業",
    "F-②電腦、電子產品及光學製品製造業",
    "F-③汽車及其零件製造業",
...
# Keep only allowed tables that actually exist in this dataset
allowed_present = [t for t in table_names if t in allowed_tables]

# (Optional) Fallback for names with suffixes (e.g., GUIDs)
if not allowed_present:
    for t in table_names:
        for a in allowed_tables:
            if t.startswith(a):
                allowed_present.append(t)
    # dedupe while preserving order
    seen = set()
    allowed_present = [t for t in allowed_present if not (t in seen or seen.add(t))]
...
# Multi-select tables + TOPN picker (restricted)
