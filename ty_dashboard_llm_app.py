...
# ─────────────────────────────────────────────────────────────
# Static workspace (TYCGEDB FORMAL). You can change if needed.
WORKSPACE_ID = "41bfeb46-6ff1-4baa-824a-9681be3a586d"
PBI_BASE = "https://api.powerbi.com/v1.0/myorg"

# ─────────────────────────────────────────────────────────────
# Sidebar credentials (UI only, as requested)
...
def get_fabric_token(tenant_id: str, client_id: str, client_secret: str) -> str:
    # Fabric token via OAuth2 client credentials
...
# ─────────────────────────────────────────────────────────────
# Safe code execution for GPT (no OS/files)
def run_safe_python(code_str: str, local_vars: dict):
    ...
    if any(s in code_str for s in forbidden_snippets):
...
# ─────────────────────────────────────────────────────────────
# UI
st.title("📊 Power BI → Fabric Tables → GPT-4o (multi-turn, safe code)")
...
# Pick dataset (dynamic list from REST)
...
# Discover tables via Fabric model (reliable for Import/DirectQuery)
...
# Fetch tables (cached), preview, download
...
# Compute common columns and suggest an X-axis
...
# ─────────────────────────────────────────────────────────────
# Multi-turn chat with GPT-4o (and safe code execution)
if "chat" not in st.session_state:
    st.session_state.chat = [
        {"role": "system", "content":
         "You are a helpful data analyst. "
         "You have access to pandas DataFrames in a dict named `data_dict` (keys are table names). "
         "If you generate Python code, wrap it in triple backticks with 'python'. "
         "When charting multiple tables, prefer using a common x-axis if provided in `x_axis`."}
    ]
...
user_q = st.text_area("Your question / request:", placeholder="e.g., Plot a line chart comparing totals by Month across the selected tables.")
...
if run_btn and user_q.strip():
    # Add user message with compact data context for grounding
...
# After potential rerun, look at the last assistant message and execute code if present
if st.session_state.chat and st.session_state.chat[-1]["role"] == "assistant":
    ...
    if code_match:
        ...
        # Provide plotting-friendly locals: pandas, Streamlit, and the data dict + x_axis
        safe_locals = {
            "pd": pd,
            "st": st,
            "data_dict": dfs,
        ...
        run_safe_python(code_block, safe_locals)
