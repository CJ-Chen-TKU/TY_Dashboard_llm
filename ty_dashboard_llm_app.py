...
FONT_DIR  = os.path.expanduser("~/.fonts")  # or "./.fonts" if that's your folder
FONT_PATH = os.path.join(FONT_DIR, "NotoSansTC-Regular.otf")  # change to .ttf if needed
...
# ─────────────────────────────────────────────────────────────
# Matplotlib Chinese font helpers required by the HARD RULES
def enable_matplotlib_chinese(font_path: str) -> bool:
    try:
        import matplotlib.pyplot as plt
        from matplotlib import font_manager as fm
        fm.fontManager.addfont(font_path)
        plt.rcParams["font.sans-serif"] = ["Noto Sans TC", "Arial", "DejaVu Sans"]
...
ensure_font()
st.info(f"Using Chinese font at: {os.path.abspath(FONT_PATH)}")
