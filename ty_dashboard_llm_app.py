    ...
    if not user_q.strip():
        ...
        if "```python" in reply.lower():
            ...
            try:
                ...
                out = pdf.output(dest="S")  # str or bytes depending on fpdf2 version
            ...
            # Only show download if we have bytes (timestamped filename)
            if pdf_bytes:
                ts = (
                    datetime.now(tz).strftime("%Y%m%d_%H%M%S")
                    if tz else datetime.now().strftime("%Y%m%d_%H%M%S")
                )
