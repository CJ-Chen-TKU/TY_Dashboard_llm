...
def run_safe_python(code_str: str, local_vars: dict, allow_autoinstall: bool = False):
    ...
    with contextlib.redirect_stdout(io.StringIO()) as buf:
        try:
            ...
            # Typically undefined variable (invented DataFrame name)
            missing_name = str(e).split("'")[1] if "'" in str(e) else str(e)
            show_var_help(missing_name)
            return

        except KeyError as e:
            # Often a missing column name in a DataFrame OR dict-like access
            missing_key = str(e).strip("'").strip('"')
            # Heuristic: if it looks like a column label (letters/numbers/spaces/_-), show column help
...
# 1) Pick dataset (dynamic from REST) -> defines dataset_id
...
dataset_id = datasets[ds_idx]["id"]  # <-- defined here

# 2) Discover tables via Fabric model for that dataset
...
# 3) Restrict to your allowed tables (exact names; with fallback for suffixes)
allowed_tables = {
    "F-①電子零組件製造業",
    "F-②電腦、電子產品及光學製品製造業",
    "F-③汽車及其零件製造業",
...
# keep exact matches
allowed_present = [t for t in table_names if t in allowed_tables]

# fallback: accept names that start with allowed label (handles suffixes/IDs)
if not allowed_present:
    tmp = []
    for t in table_names:
        for a in allowed_tables:
            if t.startswith(a):
                tmp.append(t)
    # dedupe while preserving order
    seen = set()
    allowed_present = [t for t in tmp if not (t in seen or seen.add(t))]

if not allowed_present:
    st.error(
        "None of the allowed tables are present in this dataset.
...
# 4) Multi-select tables + TOPN (restricted to allowed tables found)
picked_tables = st.multiselect(
    "Select tables",
    options=allowed_present,
    default=allowed_present[:1]
)
...
if ask_clicked:
    if not user_q.strip():
        ...
        # If the assistant returned code, let the executor section handle it
        if "```python" in reply.lower():
            st.session_state.is_report = False
            st.rerun()

        # Otherwise treat it as a narrative report
        ...
        # Build the PDF in memory with Chinese font
        pdf_bytes = None
        try:
            from fpdf import FPDF
            from datetime import datetime
        ...
        # Show a timestamped download button only if we have bytes
        if pdf_bytes:
            ts_local = (
                datetime.now(tz).strftime("%Y%m%d_%H%M%S")
                if 'tz' in locals() and tz else datetime.now().strftime("%Y%m%d_%H%M%S")
            )
            ds_name = datasets[ds_idx]["name"] if datasets else "dataset"
            fname = f"{ds_name}_report_{ts_local}.pdf"
            st.download_button(
