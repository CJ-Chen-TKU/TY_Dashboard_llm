...
# ─────────────────────────────────────────────────────────────
# UI
st.title("📊 Power BI → Fabric Tables → GPT-4o")
...
if ask_clicked:
    if not user_q.strip():
        ...
        # Build payload with sample rows + table/column hints
        ...
        # force next rerun to execute this brand‑new assistant message
        st.session_state.exec_sig = None
        st.rerun()

# --- After rerun: execute ALL python code blocks from the last assistant message, once ---
if st.session_state.chat and st.session_state.chat[-1]["role"] == "assistant":
    last_msg = st.session_state.chat[-1]["content"]

    # Simple signature to prevent re-executing the same reply on every Streamlit rerun
    if "exec_sig" not in st.session_state:
        st.session_state.exec_sig = None
    sig = hash(last_msg)

    if st.session_state.exec_sig != sig:
        # 1) Render narrative text (assistant content minus code blocks)
        narrative = re.sub(r"```python.*?```", "", last_msg, flags=re.S | re.I).strip()
        if narrative:
            st.markdown("### 📝 LLM Notes")
            st.markdown(narrative)

        # 2) Find and execute ALL python code blocks in order
        code_blocks = re.findall(r"```python\s*(.*?)```", last_msg, flags=re.S | re.I)
        if code_blocks:
            for i, block in enumerate(code_blocks, 1):
                st.markdown(f"**🛠 Executing code block {i}/{len(code_blocks)}…**")
                run_safe_python(block, safe_locals, allow_autoinstall=ALLOW_AUTOINSTALL)
        else:
            st.info("No Python code found in the response.")

        # Mark this reply as executed so it won't run again until a new assistant message arrives
        st.session_state.exec_sig = sig
