...
# Multi-turn chat (strict plotting rules)
...
if reset_clicked:
    ...
    # also clear any previous report
...
if ask_clicked:
    if not user_q.strip():
        ...
        # Heuristic: if assistant included a python code fence, treat as chart/code.
        if "```python" in reply.lower():
            st.session_state.is_report = False
            st.rerun()  # let the executor section pick it up
        else:
            # Treat as narrative report (Markdown). Store and render.
            ...
            # Ensure fpdf2 (module 'fpdf') is available to export PDF (no filesystem writes).
            try:
                import importlib
                importlib.import_module("fpdf")   # <-- module name is 'fpdf'
            except Exception:
...
# Build PDF in memory (supports Chinese via FONT_PATH)
pdf_bytes = None
try:
    from fpdf import FPDF
    from datetime import datetime
...
# Only render the download button if we have bytes
if pdf_bytes:
    ts = (
    ...
    fname = f"{ds_name}_report_{ts}.pdf"
    st.download_button(
        "⬇️ Download report.pdf",
        data=pdf_bytes,
        file_name=fname,
